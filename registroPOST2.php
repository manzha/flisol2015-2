            ...
            <div class="row-fluid">
                ...
                if (isset($_POST['registrar'])) {
                    ...
                    if (empty($strNombre) || empty($strCorreo) || empty($strGenero) || empty($strOcupacion) || empty($strEscuela)) {
                        ...
                        echo "<br>Para regresar por favor de <a href='index.php'>clic aqui</a>";
                    } else {
                        //Validacion emails
                        if (filter_var($strCorreo, FILTER_VALIDATE_EMAIL)) {
                            $query = sprintf("SELECT correo FROM flisol_asistentes WHERE correo = '$strCorreo'");
...
//Validar universidad
                                if (validarInstituto($strEscuela)) {
                                    ...
                                    $query = sprintf("INSERT INTO flisol_asistentes (nombre, correo, genero, ocupacion, escuela, comentarios,fecha)
VALUES ('$strNombre','$strCorreo','$strGenero','$strOcupacion','$strEscuela','$strComentarios',2013)");
                                    $result = mysql_query($query);
                                    $N = count($arrTalleres);
...
//Enviamos emial de que ya quedo todo listo.
                                    ...
                                    $mensaje = 'El registro al FLISoL Madero 2013 se ha realizado de manera exitosa. Te estaremos notificando por este medio de cualquier cambio significante en el evento.';
                                    ...
                                    $mensaje = 'Un asistente se ha registrado al FLISoL Madero 2013. 
